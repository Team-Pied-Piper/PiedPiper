...
//Olvide mi password / Recuperar password
exports.forgotPassword = catchAsyncErrors(async (req, res, next) => {
    ...
    //Crear una url para hacer el rest del password
    const resetUrl = `${req.protocol}://${req.get("host")}/api/resetPassword/${resetToken}`;

    const resetMessage = `El link para establecer una nueva contraseña es el siguiente: \n\n${resetUrl}\n\n
    Si no solicito este link, comuniquese con soporte. \n\n Att:\nPied Piper`

    try {
        await sendEmail({
            email:user.email,
            subject: "Pied Piper - Recuperar Contraseña",
            resetMessage
        })
        res.status(200).json({
            success:true,
...
exports.updatePassword= catchAsyncErrors(async (req, res, next) =>{
    ...
    const sonIguales = await user.comparePassword(req.body.oldPassword)
